        ...
        <div className='h-full w-full overflow-y-scroll flex justify-center items-center bg-darkGreen p-[5px]'>
            <div className='h-full w-full bg-dark flex flex-col'>
                {session && session.history.length > 0 && session.history.map((game, i) => {
                    return <div className='h-1/6 w-full flex items-center gap-[2px] p-[4px] greenUnderline bg-dark'>
                        ...
                        <div className='h-full w-1/6 bg-darkGreen text-[12px] text-center'>User <span className='bg-dark text-lightGreen flex justify-center'>{game.userNumbers && game.userNumbers.map((num) => { return num })}</span></div>
                        <div className='h-full w-1/6 bg-darkGreen text-[12px] text-center'>Winner <span className='bg-dark text-lightGreen flex justify-center'>{game.winningNums && game.winningNums.map((num) => { return num })}</span></div>
...
// all 3 any and all 4 any not paying out check session 1 numbers