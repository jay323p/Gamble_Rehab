...
const KenoPlayBox = () => {
  ...
  const [choiceRecordKeep, setChoiceRecordKeep] = useState({});
  const [winnersRecordKeep, setWinnersRecordKeep] = useState({});
  const [doneChoose, setDoneChoosing] = useState(false);
  const [gameStart, setGameStart] = useState(false);
  const [gameDone, setGameDone] = useState(false);
  const [simData, setSimData] = useState(initialSimData);
  const [regularGameMatches, setRegularGameMatches] = useState(0);
  ...
  const updateSpotsArray = (e) => {
    ...
    if (
      ...
      if (spotCounter === spots.length - 1) {
        setChoiceRecordKeep(tempObject);
  ...
  const playKenoGame = async (replayFixer) => {
    ...
    console.log('playing KENO GAME _______________________');
    ...
    const gameInterval = setInterval(() => {
      if (!readyToChooseSpotsRedux) {
        console.log('gameInterval cleared');
        setReady(false);
        setKenoGame({});
        setSpots([]);
        setTempObject({});
      ...
      if (num in numCheck) {
        ...
        if (winningBtnClass) {
          ...
          setRegularGameMatches((prev) => prev + 1);
          matches += 1;
          totalMatches += 1;
          setSimData((simData) => ({
            ...simData,
      ...
      if (winningNums.length === 20 && kenoGame.mode === 'Regular') {
        setGameDone(true); // regular keno game done {logic}
        setKenoGame({
          ...kenoGame,
  ...
  useEffect(() => {
    if (readyToChooseSpotsRedux) {
      ...
      if (!gameStart && !replayKenoGameRedux) {
        ...
        console.log('in hereeeeeeeeeeeeeeeee');
        setKenoGame({
          ...kenoGame,
          mode: previousKenoGameRedux.mode,
          spots: previousKenoGameRedux.spots,
    ...
    <>
      {showKenoSimHistoryModalRedux ? (
        ...
        <div className="h-full w-full flex flex-col ">
          ...
          <div
            className={`${
              ready ? 'w-full h-full' : 'w-full h-5/6'
            } flex justify-center items-center`}
          >
