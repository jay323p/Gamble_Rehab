...
const SlotsPlayBox = () => {
  ...
  const [slotMachineDimensions, setSlotMachineDimensions] = useState({});
  ...
  const getSlotMachineDimensions = () => {
    ...
    let fixedWidth = Math.floor(slotMachine.clientWidth / 3);
    setSlotMachineDimensions({
      height: fixedHeight,
      width: fixedWidth,
    });
  };
    ...
    for (let i = 0; i < rowMapper.length; i++) {
      ...
      for (let j = 0; j < rowMapper[i].length; j++) {
        ...
        if (matchesCounter === 2) {
          ...
          // prevWagered += 1
          // prevWon += filteredWinner[0].payout;
          // prevProfit += ()
          ...
          {slotImagesMapper.map((slotImg, i) => {
            return (
              <img
                key={slotImg.id}
                src={slotImg.src}
                className={`w-full greenUnderline object-cover xl:object-contain ${
                  (row1.length > 0 && row1[0] === i) ||
                  row2[0] === i ||
                  row3[0] === i
                    ? ' brightness-200'
                    : ''
